import { motion } from 'framer-motion';
import { Radio, Gavel, Gift, Search, Bot, ArrowLeft, ArrowRight } from 'lucide-react';
import { useApp } from '../../contexts/AppContext';
import { telegramLinks } from '../../data/translations';
import TiltCard from '../TiltCard';

const containerVariants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.05,
      delayChildren: 0.05
    }
  },
  exit: { opacity: 0 }
};

const itemVariants = {
  hidden: { y: 15, opacity: 0 },
  visible: {
    y: 0,
    opacity: 1,
    transition: { duration: 0.25, ease: 'easeOut' }
  }
};

export default function ChannelsSection() {
  const { t, navigateTo, isDark, lang, playClickSound } = useApp();
  const Arrow = lang === 'ar' ? ArrowRight : ArrowLeft;

  const buttons = [
    { id: 'superton', label: t.sections.superton, icon: Radio },
    { id: 'auction', label: t.sections.auction, icon: Gavel },
    { id: 'gifts', label: t.sections.gifts, icon: Gift }
  ];

  return (
    <motion.section
      variants={containerVariants}
      initial="hidden"
      animate="visible"
      exit="exit"
      style={{
        width: 'min(92%, 820px)',
        textAlign: 'center',
        padding: '12px 10px 86px',
        marginInline: 'auto'
      }}
    >
      {/* Title */}
      <motion.h1
        variants={itemVariants}
        style={{
          fontSize: 'clamp(1.8rem, 3vw, 2.6rem)',
          margin: '20px 0',
          textShadow: '0 2px 8px rgba(0,0,0,.6)',
          lineHeight: 1.4,
          fontWeight: 700
        }}
      >
        {t.sections.channels}
      </motion.h1>

      {/* Sub-section Buttons */}
      {buttons.map((button) => (
        <TiltCard
          key={button.id}
          variants={itemVariants}
          onClick={() => navigateTo(button.id)}
          whileHover={{ 
            scale: 1.02, 
            y: -2,
            boxShadow: isDark 
              ? '0 0 25px rgba(0,180,255,.5)' 
              : '0 14px 30px rgba(0,0,0,.3), 0 0 18px rgba(0,180,255,.25)'
          }}
          whileTap={{ scale: 0.98 }}
          style={{
            display: 'flex',
            width: 'min(360px, 90%)',
            margin: '10px auto',
            padding: 'clamp(12px, 1.5vw, 18px) clamp(20px, 2vw, 28px)',
            fontSize: 'clamp(1rem, 1.5vw, 1.2rem)',
            color: '#fff',
            cursor: 'pointer',
            borderRadius: '12px',
            border: `1px solid ${isDark ? 'rgba(0,180,255,.3)' : 'rgba(255,255,255,.28)'}`,
            background: isDark ? 'rgba(0,150,255,.15)' : 'rgba(255,255,255,.12)',
            backdropFilter: 'blur(8px)',
            boxShadow: '0 10px 26px rgba(0,0,0,.25)',
            fontFamily: 'inherit',
            alignItems: 'center',
            justifyContent: 'center',
            gap: '12px'
          }}
        >
          <button.icon size={22} />
          {button.label}
        </TiltCard>
      ))}

      {/* Platform Bot Card */}
      <TiltCard
        variants={itemVariants}
        as="div"
        style={{
          width: 'min(360px, 90%)',
          margin: '10px auto',
          padding: 'clamp(16px, 2vw, 20px)',
          borderRadius: '16px',
          border: `1px solid ${isDark ? 'rgba(0,180,255,.4)' : 'rgba(0,180,255,.5)'}`,
          background: isDark ? 'rgba(0,150,255,.12)' : 'rgba(0,150,255,.15)',
          backdropFilter: 'blur(8px)',
          boxShadow: '0 10px 26px rgba(0,0,0,.25)',
          display: 'block',
          textAlign: 'start'
        }}
      >
        <div style={{
          display: 'flex',
          alignItems: 'center',
          gap: '12px',
          marginBottom: '10px'
        }}>
          <Bot size={26} color="#fff" />
          <span style={{
            fontSize: 'clamp(1.1rem, 1.8vw, 1.3rem)',
            fontWeight: 600,
            color: '#fff'
          }}>
            {t.links.platformBot}
          </span>
        </div>
        <div style={{
          fontSize: '0.9rem',
          color: 'rgba(255,255,255,.75)',
          marginBottom: '14px',
          lineHeight: 1.5
        }}>
          {t.links.platformBotDesc}
        </div>
        <motion.a
          href={telegramLinks.platformBot}
          target="_blank"
          rel="noopener noreferrer"
          whileHover={{ 
            scale: 1.03,
            boxShadow: '0 6px 20px rgba(0,150,255,.4)'
          }}
          whileTap={{ scale: 0.97 }}
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            justifyContent: 'center',
            padding: '10px 24px',
            fontSize: '0.95rem',
            fontWeight: 600,
            color: '#fff',
            background: 'linear-gradient(135deg, rgba(0,180,255,.8), rgba(0,120,200,.8))',
            borderRadius: '10px',
            textDecoration: 'none',
            cursor: 'pointer',
            border: '1px solid rgba(255,255,255,.2)',
            boxShadow: '0 4px 15px rgba(0,0,0,.2)'
          }}
        >
          {lang === 'ar' ? 'الدخول للبوت →' : 'Enter Bot →'}
        </motion.a>
      </TiltCard>

      {/* Scammers Bot Link */}
      <TiltCard
        variants={itemVariants}
        href={telegramLinks.scammersBot}
        target="_blank"
        rel="noopener noreferrer"
        onClick={playClickSound}
        whileHover={{ 
          scale: 1.02, 
          y: -2,
          boxShadow: isDark 
            ? '0 0 25px rgba(255,100,100,.4)' 
            : '0 14px 30px rgba(0,0,0,.3)'
        }}
        whileTap={{ scale: 0.98 }}
        style={{
          display: 'flex',
          width: 'min(360px, 90%)',
          margin: '10px auto',
          padding: 'clamp(12px, 1.5vw, 18px) clamp(20px, 2vw, 28px)',
          fontSize: 'clamp(1rem, 1.5vw, 1.2rem)',
          color: '#fff',
          cursor: 'pointer',
          borderRadius: '12px',
          border: `1px solid ${isDark ? 'rgba(255,100,100,.4)' : 'rgba(255,100,100,.5)'}`,
          background: isDark ? 'rgba(255,100,100,.15)' : 'rgba(255,100,100,.2)',
          backdropFilter: 'blur(8px)',
          boxShadow: '0 10px 26px rgba(0,0,0,.25)',
          fontFamily: 'inherit',
          textDecoration: 'none',
          alignItems: 'center',
          justifyContent: 'center',
          gap: '12px'
        }}
      >
        <Search size={22} />
        {t.links.scammers}
      </TiltCard>

      {/* Back Button */}
      <motion.button
        variants={itemVariants}
        onClick={() => navigateTo('main')}
        whileHover={{ 
          scale: 1.02, 
          y: -2,
          boxShadow: '0 14px 30px rgba(0,0,0,.3)'
        }}
        whileTap={{ scale: 0.98 }}
        style={{
          display: 'flex',
          width: 'min(360px, 90%)',
          margin: '20px auto 12px',
          padding: 'clamp(12px, 1.5vw, 18px) clamp(20px, 2vw, 28px)',
          fontSize: 'clamp(1rem, 1.5vw, 1.2rem)',
          color: 'rgba(255,255,255,.8)',
          cursor: 'pointer',
          borderRadius: '12px',
          border: `1px solid ${isDark ? 'rgba(255,255,255,.2)' : 'rgba(255,255,255,.2)'}`,
          background: isDark ? 'rgba(255,255,255,.08)' : 'rgba(255,255,255,.08)',
          backdropFilter: 'blur(8px)',
          boxShadow: '0 10px 26px rgba(0,0,0,.25)',
          transition: 'all 0.25s ease',
          fontFamily: 'inherit',
          alignItems: 'center',
          justifyContent: 'center',
          gap: '12px'
        }}
      >
        <Arrow size={20} />
        {t.buttons.back}
      </motion.button>
    </motion.section>
  );
}
